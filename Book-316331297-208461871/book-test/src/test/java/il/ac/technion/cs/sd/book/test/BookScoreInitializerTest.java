...
public class BookScoreInitializerTest {
	...
	public void test() {
		...
		init.setup("<Root>" + "<Reviewer Id=\"123\">" + "<Review>" + "<Id>Foobar</Id>" + "<Score>10</Score>"
				...
				+ "</Reviewer>" + "</Root>");
		try {
			assertEquals(init.getDict().findByKeys("123", "Foobar"), Optional.of("5"));
	...
	public void test1() {
		...
		init.setup("<Root>" + "<Reviewer Id=\"123\">" + "<Review>" + "<Id>Foobar</Id>" + "<Score>10</Score>"
				...
				+ "</Root>");
		try {
			assertEquals(init.getDict().findByKeys("678", "Boobar"), Optional.of("5"));
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void test2() {
		...
		init.setup("<Root>" + "<Reviewer Id=\"123\">" + "<Review>" + "<Id>Foobar</Id>" + "<Score>10</Score>"
				...
				+ "</Root>");
		try {
			assertEquals(init.getDict().findByKeys("678", "bush"), Optional.of("9"));
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
