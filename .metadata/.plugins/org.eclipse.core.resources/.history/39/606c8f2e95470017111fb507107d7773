...
public class BookScoreReaderImpl implements BookScoreReader {
	...
	public List<String> getReviewedBooks(String reviewerId) {
		try {
			return dict.findByMainKey(reviewerId).stream().map(Pair::getKey).sorted().collect(Collectors.toList());
		} catch (InterruptedException e) {
			throw new RuntimeException();
		}
	}
	...
	public List<String> getReviewers(String bookId) {
		try {
			return dict.findBySecondaryKey(bookId).stream().map(Pair::getKey).sorted().collect(Collectors.toList());
		} catch (InterruptedException e) {
			throw new RuntimeException();
		}
	}
